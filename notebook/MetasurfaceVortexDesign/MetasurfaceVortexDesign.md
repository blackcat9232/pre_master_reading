...
A polarization-insensitive metasurface for vortex generation based on propagation phase modulation mechanism is designed and analyzed.
...
from MetaTool import nk2permittivity, setResources, phaseDis, phaseNorm
```
...
```python
def getVortexPhaseMat(size: int, charge: int, circle: bool=False):
    """ 
    Return the 2D phase distribution matrix generating the vortex beam
    @param size: integer, the row and column number of phase matrix
    @param charge: integer, topology charge of vortex
    @param circle: the shape of phase profile is a circle but square (corner elements will be nan). Default: False
    @return: 2D phase distribution matrix (numpy array)
    """
    phase_mat = np.zeros((size, size))
    offset = size / 2 - 1 / 2
    if not circle:
        for i in range(size):
            for j in range(size):
                x = j - offset
                y = i - offset
                phase_mat[i, j] = charge * np.arctan2(y, x)
    else:
        for i in range(size):
            for j in range(size):
                x = j - offset
...
Here we design a polarization-insensitive metasurface for vortex generation using the cylinder meta-atoms. 
...
size = 8  # size of metasurface in unit number along x and y direction
charge = 1  # topology charge number for vortex generating
multiple = 1  # multiple the phase distribution
phase_profile_dest = getVortexPhaseMat(size, charge)
phase_profile_dest = phaseNorm(getVortexPhaseMat(size, charge))
phase_profile_dest = np.tile(phase_profile_dest, (multiple, multiple))
...
```python
fdtd.save("../fsp/metasurface_vortex.fsp")  # save simulation
```
...
```
    ...
    Run time cost: 11.915826500000001s.
